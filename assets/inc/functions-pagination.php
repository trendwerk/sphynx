...
function tp_pagination( $args = null ) {
	...
	if( 1 < $pages ) {
		...
		if( $left_gap && ! $right_gap )
			$output .= sprintf( '%s%s%s', tp_pagination_loop( 1, $anchor ), $ellipsis, tp_pagination_loop( $block_min, $pages, $page ) );
		elseif( $left_gap && $right_gap )
			$output .= sprintf( '%s%s%s%s%s', tp_pagination_loop( 1, $anchor ), $ellipsis, tp_pagination_loop( $block_min, $block_high, $page ), $ellipsis, tp_pagination_loop( ( $pages - $anchor + 1 ), $pages ) );
		elseif( $right_gap && ! $left_gap )
			$output .= sprintf( '%s%s%s', tp_pagination_loop( 1, $block_high, $page ), $ellipsis, tp_pagination_loop( ( $pages - $anchor + 1 ), $pages ) );
		else
...
function tp_pagination_loop( $start, $max, $page = 0 ) {
	...
	for( $i = $start; $i <= $max; $i++ )
		$output .= ( $page === intval( $i ) ) ? '<li><span class="page current">' . $i . '</span></li>'  : '<li><a href="' . get_pagenum_link( $i ) . '" class="page">' . $i . '</a></li>';
